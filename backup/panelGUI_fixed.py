...
class MotorPanel(QtWidgets.QGroupBox):
    ...
    def _on_motor_selected(self, sn: str) -> None:
        # Emit selection to worker; update UI hint
        self.motorSelected.emit(sn)
        if sn and "not selected" not in sn.lower():
            self.set_motor_status("warn", f"SELECTED {sn}")
        else:
            self.set_motor_status("off", "OFF")
...
class WeightPowerPanel(QtWidgets.QGroupBox):
    ...
    def _on_zero_clicked(self) -> None:
        ...
        # GUI hint: update display immediately
        self._zero()
...
#  --- GUI-only periodic logging ---
    ...
    def _init_workers(self) -> None:
        ...
        self.motor_panel.pauseToggled.connect(self._on_pause_toggled)
        ...
        # --- workers -> GUI ---
        self.motor_worker.connectedChanged.connect(self._on_motor_connected)
        self.motor_worker.stateChanged.connect(self._on_motor_state)
        self.motor_worker.telemetryChanged.connect(self._on_motor_telemetry)
        self.motor_worker.error.connect(self._on_motor_error)

        self.scale_worker.connectedChanged.connect(self._on_scale_connected)
        self.scale_worker.telemetryChanged.connect(self._on_scale_telemetry)
        self.scale_worker.error.connect(self._on_scale_error)

    @QtCore.Slot(bool)
    def _on_pause_toggled(self, paused: bool) -> None:
        # Ensure pause/resume runs in motor thread
        method = "pause" if paused else "resume"
        QtCore.QMetaObject.invokeMethod(self.motor_worker, method, QtCore.Qt.ConnectionType.QueuedConnection)

    @QtCore.Slot(bool)
    def _on_motor_connected(self, ok: bool) -> None:
        if ok:
            self.motor_panel.set_motor_status("ok", "IDLE")
        else:
            self.motor_panel.set_motor_status("off", "DISCONNECTED")

    @QtCore.Slot(str)
    def _on_motor_state(self, state: str) -> None:
        s = (state or "").upper()
        if s == "RUNNING":
            self.motor_panel.set_motor_status("ok", "RUNNING")
        elif s == "PAUSED":
            self.motor_panel.set_motor_status("warn", "PAUSED")
        elif s == "ERROR":
            self.motor_panel.set_motor_status("err", "ERROR")
        elif s == "DISCONNECTED":
            self.motor_panel.set_motor_status("off", "DISCONNECTED")
        else:
            self.motor_panel.set_motor_status("warn", s or "UNKNOWN")

    @QtCore.Slot(dict)
    def _on_motor_telemetry(self, tel: dict) -> None:
    ...
    @QtCore.Slot(str)
    def _on_motor_error(self, msg: str) -> None:
        self.statusBar().showMessage(f"Motor error: {msg}")
        self.motor_panel.set_motor_status("err", "ERROR")

    @QtCore.Slot(bool)
    def _on_scale_connected(self, ok: bool) -> None:
        if ok:
    ...
    @QtCore.Slot(dict)
    def _on_scale_telemetry(self, tel: dict) -> None:
        if not isinstance(tel, dict):
            return
    ...
    @QtCore.Slot(str)
    def _on_scale_error(self, msg: str) -> None:
...
def main():
    try:
        ...
        w.show()
        exit_code = app.exec()
        print(f"SCADA GUI application exited with code {exit_code}.")
        sys.exit(exit_code)
